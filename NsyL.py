...
def _Fetch(guild_id):
	...
	try:
		...
		params = json.dumps({'guild':guild_id,'token':token,'charname':character_data['name'],'fetch_all':QtBind.isChecked(gui,cbxDiscord_check_all)}).encode('utf8')
		if not url.endswith("/"):
			url += "/"
...
def joined_game():
	loadConfigs()
	Notify(QtBind.text(gui,cmbxEvtChar_joined),"|`"+character_data['name']+"`| - Joined to the game",colour=0xFF00FF)
...
def disconnected():
	...
	if isOnline:
		...
		if channel_id:
			Notify(channel_id,"|`"+character_data['name']+"`| You has been disconnected",colour=0x00DDFF)
...
def handle_chat(t,player,msg):
	...
	# Check message type
	if t == 1:
		Notify(QtBind.text(gui_,cmbxEvtMessage_all),"|`"+character_data['name']+"`| - [General] from `"+player+"`: "+msg,colour=0x00FF00)
	elif t == 2:
		Notify(QtBind.text(gui_,cmbxEvtMessage_private),"|`"+character_data['name']+"`| - [**Private**] from `"+player+"`: "+msg,colour=0x00FFFF)
	elif t == 9:
		Notify(QtBind.text(gui_,cmbxEvtMessage_stall),"|`"+character_data['name']+"`| - [**Stall**] from `"+player+"`: "+msg,colour=0x00FF00)
	elif t == 4:
		Notify(QtBind.text(gui_,cmbxEvtMessage_party),"|`"+character_data['name']+"`| - "+"[**Party**] `"+player+"`: "+msg,colour=0x0000FF)
	elif t == 16:
		Notify(QtBind.text(gui_,cmbxEvtMessage_academy),"|`"+character_data['name']+"`| - "+"[**Academy**] `"+player+"`: "+msg,colour=0x00DDFF)
	elif t == 5:
		Notify(QtBind.text(gui_,cmbxEvtMessage_guild),"`"+player+"`: "+msg,colour=0xFF9900)
	elif t == 11:
		Notify(QtBind.text(gui_,cmbxEvtMessage_union),"[**Union**] `"+player+"`: "+msg,colour=0x00FF00)
	elif t == 6:
			...
			if searchMessage:
				try:
					if re.search(searchMessage,msg):
						Notify(QtBind.text(gui_,cmbxEvtMessage_global),"   `"+player+"`: **"+msg+"**",colour=0xFFFF00)
				except Exception as ex:
		...
		if QtBind.isChecked(gui_,cbxEvtMessage_notice_filter):
			...
			if searchMessage:
				try:
					if re.search(searchMessage,msg):
						Notify(QtBind.text(gui_,cmbxEvtMessage_notice),"[**Alchemy**] : "+msg,colour=0xFF00FF)
				except Exception as ex:
					log("Plugin: Error at regex ["+str(ex)+"]")
		else:
			Notify(QtBind.text(gui_,cmbxEvtMessage_notice),"[**Alchemy**] : "+msg,colour=0xFF00FF)
	elif t == 3:
		Notify(QtBind.text(gui_,cmbxEvtMessage_gm),"[GameMaster] "+player+": "+msg,colour=0xFF00FF)
...
def handle_event(t, data):
	...
	if t == 9:
	...
	elif t == 1: # Hunter/Trader near notification
		...
		Notify(QtBind.text(gui,cmbxEvtChar_died),"|`"+character_data['name']+"`| - You died",CreateInfo("position",get_position()),colour=0xFF5722)
	elif t == 3:
		pet = get_pets()[data]
		Notify(QtBind.text(gui,cmbxEvtPet_died),"|`"+character_data['name']+"`| - Your pet `"+(pet['type'].title())+"` died",colour=0xFF5722)
	elif t == 5:
		...
		if channel_id:
			...
			genre = getGenreText(item['servername'])
			sox = getSoXText(item['servername'],item['level'])
			Notify(channel_id,"|`"+character_data['name']+"`| - **Item (Rare)** picked up ***"+item['name']+(' '+race if race else '')+(' '+genre if genre else '')+(' '+sox if sox else '')+"***",colour=0xFF9900)
	elif t == 6:
		...
		if channel_id:
			...
			genre = getGenreText(item['servername'])
			sox = getSoXText(item['servername'],item['level'])
			Notify(channel_id,"|`"+character_data['name']+"`| - **Item (Equipable)** picked up ***"+item['name']+(' '+race if race else '')+(' '+genre if genre else '')+(' '+sox if sox else '')+"***",colour=0xFF9900)
	elif t == 8:
		Notify(QtBind.text(gui,cmbxEvtBot_alchemy),"|`"+character_data['name']+"`| - **Auto alchemy** has been completed",colour=0x0000FF)
...
def handle_joymax(opcode, data):
	...
	# SERVER_BA_NOTICE
		...
		if channel_id:
			updateType = data[0]
			if updateType == 2:
				Notify(channel_id,"[**Battle Arena**] ("+getBattleArenaText(data[1])+") will start at 45 minutes",colour=0xFF0000)
			elif updateType == 13:
				Notify(channel_id,"[**Battle Arena**] ("+getBattleArenaText(data[1])+") will start at 5 minutes",colour=0xFF0000)
			elif updateType == 14:
				Notify(channel_id,"[**Battle Arena**] ("+getBattleArenaText(data[1])+") will start at 1 minute",colour=0xFF0000)
			elif updateType == 3:
				Notify(channel_id,"[**Battle Arena**] registration period has ended",colour=0xFF0000)
			elif updateType == 4:
				Notify(channel_id,"[**Battle Arena**] started",colour=0xFF0000)
			elif updateType == 5:
				Notify(channel_id,"[**Battle Arena**] has ended",colour=0xFF0000)
	# SERVER_CTF_NOTICE
	elif opcode == 0x34B1:
		...
		if channel_id:
			updateType = data[0]
			if updateType == 2:
				Notify(channel_id,"[**Capture the Flag**] will start at 10 minutes",colour=0xFF0000)
			elif updateType == 13:
				Notify(channel_id,"[**Capture the Flag**] will start at 5 minutes",colour=0xFF0000)
			elif updateType == 14:
				Notify(channel_id,"[**Capture the Flag**] will start at 1 minute",colour=0xFF0000)
			elif updateType == 3:
				Notify(channel_id,"[**Capture the Flag**] started",colour=0xFF0000)
			elif updateType == 9:
				Notify(channel_id,"[**Capture the Flag**] has ended",colour=0xFF0000)
	# SERVER_QUEST_UPDATE
	elif opcode == 0x30D5:
		...
		if channel_id:
			...
			if data[0] == 2 and data[10] == 2:
				...
				Notify(channel_id,"|`"+character_data['name']+"`| - **Quest** has been completed ***"+quest['name']+"***",colour=0xFF5722)
	...
	# SERVER_FW_NOTICE
		...
		if channel_id:
			updateType = data[0]
			if updateType == 1:
				Notify(channel_id,"[**Fortress War**] will start in 30 minutes",colour=0xFFFF00)
			elif updateType == 2:
				Notify(channel_id,"[**Fortress War**] started",colour=0xFFFF00)
			elif updateType == 3:
				Notify(channel_id,"[**Fortress War**] has 30 minutes before the end",colour=0xFFFF00)
			elif updateType == 4:
				Notify(channel_id,"[**Fortress War**] has 20 minutes before the end",colour=0xFFFF00)
			elif updateType == 5:
				Notify(channel_id,"[**Fortress War**] has 10 minutes before the end",colour=0xFFFF00)
			elif updateType == 8:
				...
				Notify(channel_id,"[**Fortress War**] "+getFortressText(fortressID)+" has been taken by `"+guildName+"`",colour=0xFFFF00)
			elif updateType == 9:
				Notify(channel_id,"[**Fortress War**] has 1 minute before the end",colour=0xFFFF00)
			elif updateType == 6:
				Notify(channel_id,"[**Fortress War**] has ended",colour=0xFFFF00)
	# SERVER_PARTY_DATA
	elif opcode == 0x3065:
		party_data = get_party()
		channel_id = QtBind.text(gui_,cmbxEvtParty_joined)
		if channel_id:
			Notify(channel_id,"|`"+character_data['name']+"`| You has been joined to the party\n"+getPartyTextList(party_data),colour=0x00FFFF)
	# SERVER_PARTY_UPDATE
	elif opcode == 0x3864:
		updateType = data[0]
		if updateType == 1:
			Notify(QtBind.text(gui_,cmbxEvtParty_left),"|`"+character_data['name']+"`| You left the party!",colour=0x00FFFF)
		elif updateType == 2:
			party_data = get_party()
	...
	# SERVER_CHAT_ITEM_DATA
		...
		for i in range(data[1]):
			...
			try:
				...
				log('Plugin: Saving error parsing item ...')
			...
			if infoType == 2: # session
				if data[6]:
					...
					if channel_id:
						...
						Notify(channel_id,"|`"+character_data['name']+"`| Guild member `"+member['name']+"` has logged off",colour=0xFF9900)
				else:
					...
					if channel_id:
						...
						if member['name'] != character_data['name']:
							Notify(channel_id,"|`"+character_data['name']+"`|  Guild member `"+member['name']+"` has logged on",colour=0xFF9900)
...
def notify_pickup(channel_id,itemID):
	...
	if not usefilterName and not usefilterServerName:
		...
		genre = getGenreText(item['servername'])
		sox = getSoXText(item['servername'],item['level'])
		Notify(channel_id,"|`"+character_data['name']+"`| - **Item** picked up ***"+item['name']+(' '+race if race else '')+(' '+genre if genre else '')+(' '+sox if sox else '')+"***",colour=0xFF9900)
		return
	# check filter name
	if usefilterName:
		...
		if searchName:
			try:
				if re.search(searchName,item['name']):
					...
					genre = getGenreText(item['servername'])
					sox = getSoXText(item['servername'],item['level'])
					Notify(channel_id,"|`"+character_data['name']+"`| - **Item (Filtered)** picked up ***"+item['name']+(' '+race if race else '')+(' '+genre if genre else '')+(' '+sox if sox else '')+"***",colour=0xFF9900)
					return
			except Exception as ex:
