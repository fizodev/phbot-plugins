...
def btnSell_clicked():
    ...
    #
    # open_samarkand_trader_npc()
    # Timer(1.0, start_sell_goods).start()
    close_DW_trader_npc()
...
def start_buy_goods():
    ...
    while attempts < max_attempts:
        ...
        if pet_status is True:
            ...
            close_samarkand_trader_npc()
            return True

        # Buy goods, only If buy/sell paused for captcha solving
        if not buy_sell_paused_for_captcha:
            buy_samarkand_item()

        # Add a small random delay between purchases
        delay = generate_random_delay(0.5, 1.0)
...
def start_sell_goods():
    ...
    while attempts < max_attempts:
        ...
        # Buy goods, only If buy/sell paused for captcha solving
        if not buy_sell_paused_for_captcha:
            sell_samarkand_item_at_index(item_index)
        else :
...
def handle_captcha_question(data):
    global buy_sell_paused_for_captcha
...
def teleported():
    ...
    xlog(f"Current milliseconds: {current_ms}")
