...
def EditSkill():
	...
	if CheckIfOnlyMasteriesLeft():
		for Mastery in RemoveSkills:
			PotionQty = int(GetPotionCount())
			if PotionQty == 0:
				log('Plugin: No more Resuscitation Potions left')
				ExitNPC()
				return
			MasteryName = Mastery[:-8]
			MasteryLevel = GetMasteryLevel(MasteryName)
			MasteryID = GetMasteryID(MasteryName)
			# Calculate target level - can only reduce by the number of potions we have
			TargetLevel = max(0, MasteryLevel - PotionQty)
			p = b'\x59\x0E\x00\x00'
			p += struct.pack('<I', MasteryID)
			if TargetLevel == 0:
				p += b'\x00'
				log('Plugin: Mastery [%s] Removed' %MasteryName)
				QtBind.remove(gui,lstRemoveSkills,Mastery)
				SaveConfig()
			else:
				p += struct.pack('B', TargetLevel)
				log('Plugin: Reducing Mastery [%s] from Level [%s] to Level [%s]' %(MasteryName,MasteryLevel,TargetLevel))
